...
- **LIKE:** Se utiliza para buscar un patrón específico dentro de una columna de texto. Es útil cuando quieres encontrar valores que coincidan parcial o totalmente con un patrón.
- Ejemplo: 
```sql 
SELECT * FROM usuarios WHERE nombre LIKE 'Juan%';
```
- **IN:** Se utiliza para comparar el valor de una columna con una lista de valores específicos. Es útil cuando deseas filtrar filas que coincidan con alguno de los valores en una lista.
- Ejemplo: 
```sql 
SELECT * FROM usuarios WHERE pais IN ('Mexico', 'Espana', 'Argentina');
```
...
CREATE TABLE songs (
	...
	TITTLE VARCHAR2(100) NULL,
	ARTIST VARCHAR2(100) NULL, 
	DURATION TIME NULL,
	REPRODUCTION INT NULL,
	QUOTE VARCHAR2(100) NULL
);
```
